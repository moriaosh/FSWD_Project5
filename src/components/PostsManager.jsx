import { useContext, useState, useEffect } from 'react';
  ...
  function PostsManager() {
    ...
    useEffect(() => {
      fetchPosts();
    }, [currentPage, userData.posts, user.id]);

    const fetchPosts = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/posts?userId=${Number(user.id)}&_page=${currentPage}&_limit=${postsPerPage}`
        );
                    ...
                    <div className="formContainer">
                      ...
                      <form onSubmit={handleUpdateComment}>
                        <textarea
                          ...
                          className="formTextarea"
                        />
                        <button type="submit" className="formButton updateButton">Update Comment</button>
                        <button onClick={() => setEditComment(null)} className="formButton cancelButton">Cancel</button>
                      </form>
                    </div>
                  )}
                </div>
        ...
        {editPost && (
          <div className="formContainer">
            ...
            <form onSubmit={handleUpdatePost}>
              ...
              <button onClick={() => setEditPost(null)} className="formButton cancelButton">Cancel</button>
            </form>
          </div>
        )}
      </div>
